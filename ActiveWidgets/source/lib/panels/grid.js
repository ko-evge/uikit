...
AW.Panels.Grid.create = function(){
	...
	var i, rows = [top, middle, bottom], names = ["top", "center", "bottom"];
	...
	obj.changeScrollTop = function(y){
		...
		if (AW.gecko){AW.ignoreMouse = true}
		if (e1) {e1.scrollTop = y}
		if (e2) {e2.scrollTop = y}
		if (e3) {e3.scrollTop = y}
		if (AW.gecko){this.setTimeout(function(){AW.ignoreMouse = false})}
	};
	...
	obj.changePanelWidth = function(width, part){
		...
		var w = width + "px";
		var i, r, parts = ["box/top", "box/middle", "box/bottom"];
