...
export class Form extends Base {
  ...
  addField(name, config = {}) {
    ...
    // Append only the new field instead of re-rendering every field.
    // (Full render() was O(N^2) when building a form field-by-field.)
  ...
  validateField(fieldName) {
    ...
    // Built-in required check. If an explicit { type: 'required' } validator is
    // also present, honor its custom message instead of the generic default.
    if (field.required) {
    ...
    // Validators array. Each branch produces a uniform (value, formData) => error|null
    // function with the custom message already bound, so direct validators
    // (required/email/url/number/integer) and factory validators behave the same.
    if (field.validators && Array.isArray(field.validators)) {
      const msg = (validator) => validator.message;
      for (const validator of field.validators) {
        let fn = null;

        if (validator.type === 'required') {
          fn = (v) => Validators.required(v, msg(validator));
        } else if (validator.type === 'email') {
          fn = (v) => Validators.email(v, msg(validator));
        } else if (validator.type === 'url') {
          fn = (v) => Validators.url(v, msg(validator));
        } else if (validator.type === 'number') {
          fn = (v) => Validators.number(v, msg(validator));
        } else if (validator.type === 'integer') {
          fn = (v) => Validators.integer(v, msg(validator));
        } else if (validator.type === 'minLength') {
          fn = Validators.minLength(validator.min, msg(validator));
        } else if (validator.type === 'maxLength') {
          fn = Validators.maxLength(validator.max, msg(validator));
        } else if (validator.type === 'min') {
          fn = Validators.min(validator.min, msg(validator));
        } else if (validator.type === 'max') {
          fn = Validators.max(validator.max, msg(validator));
        } else if (validator.type === 'pattern') {
          fn = Validators.pattern(validator.regex, msg(validator));
        } else if (validator.type === 'custom') {
          fn = Validators.custom(validator.fn, msg(validator));
        } else if (validator.type === 'matches') {
          fn = Validators.matches(validator.field, msg(validator));
        }
  ...
  /**
   * Build (and register) the DOM group for a single field. Returns the group
   * element so callers can append it without rebuilding the whole form.
   */
  _buildField(field) {
    ...
    if (field.validationTrigger === 'change') {
      input.on('change', runValidation);
    }

    // Validation on blur
    if (field.validationTrigger === 'blur') {
      input.getDOMElement().addEventListener('blur', runValidation);
    }
