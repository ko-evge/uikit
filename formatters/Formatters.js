...
export const DateFormatter = {
  ...
  format(date, pattern = 'YYYY-MM-DD') {
    ...
    switch (pattern) {
      ...
      default: {
        // Custom pattern — один проход по всем токенам сразу.
        // Последовательные .replace() ломали результат: после MMMM → 'March'
        // следующий проход /M/g заменял 'M' внутри уже подставленного 'March'
        // ('MMMM YYYY' превращался в '3arch 2026').
        const tokens = {
        ...
        // Порядок альтернатив важен: длинные токены раньше коротких (MMMM до MM до M)
        return pattern.replace(
          /YYYY|YY|MMMM|MMM|MM|M|DD|D|dddd|ddd|HH|mm|ss/g,
          (t) => tokens[t]()
        );
      }
    }
